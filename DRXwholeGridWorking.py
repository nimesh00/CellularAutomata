...
class grid_class:
    def __init__(self):
        ...
        self.orientation = np.random.randint(lower_orientation, upper_orientation, size = (n,n))
...
def initialize_orientation():
    global state_to_orientation
    state_to_orientation = np.random.randint(lower_orientation, upper_orientation, size = (states + 1))
        ...
        for i in range(n):
            for j in range(n):
                ...
                if original_grid.dislocation_densities[i][j] >= P_cr:
                    ...
                    updation_grid.dislocation_densities[i][j] = dislocation_energy(original_grid.dislocation_densities[i][j], original_grid.orientation[i][j])
                cell_strain[i][j], _ = strain(original_grid.orientation[i][j])
