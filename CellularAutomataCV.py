...
def convert_grid_to_image(array):
    saturation = 180
    value = 180
    hue = [0] * states
    for i in range(states):
    ...
    image = np.ndarray([n, n, 3])
    image = np.array(image, dtype = np.uint8)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

    for y in range(n):
        for x in range(n):
            image[y, x, 0] = hue[grid[y][x]]
    image[:, :, 1] = saturation
    image[:, :, 2] = value
...
def main():
    ...
    while (k < iterations):
        ...
        grid_image = convert_grid_to_image(grid)

        cv2.imshow('Current Grid', grid_image)
        cv2.waitKey(30)
        # ax[0].plot(true_strain, true_stress, 'b-')
        # ax[1].plot(true_strain, xDrx, 'r-')
        # plt.pause(0.01)
