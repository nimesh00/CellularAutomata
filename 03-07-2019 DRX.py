...
class grid_class:
    def __init__(self):
        self.state = np.random.randint(1, states, size = (n,n))
...
def potential_nucleus(i, j):
    ...
    for indices in neighbour_indices:
        if (indices[0] < 0) or (indices[1] < 0) or (indices[0] > n - 1) or (indices[1] > n - 1):
            continue
        elif (original_grid.state[i][j] == original_grid.state[indices[0]][indices[1]]) and \
             (original_grid.orientation[0][0][i][j] == original_grid.orientation[0][0][indices[0]][indices[1]]) and \
    ...
    mis_orient = np.max(misorientation)
    condition1 = (mis_orient >= critical_misorientation)
...
def update_recrystallized_cell_state(i, j):
    ...
    if random_number <= nucleation_probability:
        updation_grid.state[i][j] =                            np.random.randint(1, states)
...
def consume_recrystallised_nuclei(nuclei_0, nuclei_1, grain_0, grain_1):
    ...
    for indices in neighbour_indices_nuclei:
        if (indices[0] < 0) or (indices[1] < 0) or (indices[0] > n - 1) or (indices[1] > n - 1):
            continue
        elif (original_grid.state[nuclei_0][nuclei_1] == original_grid.state[indices[0]][indices[1]]) and \
             (original_grid.orientation[0][0][nuclei_0][nuclei_1] == original_grid.orientation[0][0][indices[0]][indices[1]]) and \
             (original_grid.orientation[0][1][nuclei_0][nuclei_1] == original_grid.orientation[0][1][indices[0]][indices[1]]) and \
             (original_grid.orientation[0][2][nuclei_0][nuclei_1] == original_grid.orientation[0][2][indices[0]][indices[1]]):
            continue
        else: 
            not_similar_i += 1
    ...
    for indices in neighbour_indices_nuclei:
        if (indices[0] < 0) or (indices[1] < 0) or (indices[0] > n - 1) or (indices[1] > n - 1):
            continue
             ...
             (switch_om == original_grid.orientation[0][2][indices[0]][indices[1]]) and \
             (switch_state == original_grid.state[indices[0]][indices[1]]):
            continue
        else: 
            not_similar_f += 1
...
def propagateGrainBoundary(i, j, k, v_max):                                      #defined for a recrystallised cell to propagate (make sure to input v_max)
    ...
    for indices in neighbour_indices:
        ...
        if original_grid.dynamic_recrystallization_number[indices[0]][indices[1]] == 1:
            if (original_grid.state[i][j] == original_grid.state[indices[0]][indices[1]]) and \
               (original_grid.orientation[0][0][i][j] == original_grid.orientation[0][0][indices[0]][indices[1]]) and \
               (original_grid.orientation[0][1][i][j] == original_grid.orientation[0][1][indices[0]][indices[1]]) and \
               (original_grid.orientation[0][2][i][j] == original_grid.orientation[0][2][indices[0]][indices[1]]):
                continue
            ...
            if random_number <= propagation_probability:        
                ...
                cells_worked[indices[0]][indices[1]] = 1
...
def main():
    ...
    while (k < iterations):
        ...
        if start_process:
            for i in range(n):
                for j in range(n):
                    # if cells_worked[i][j] == 1:
                    #     continue
                    if original_grid.dynamic_recrystallization_number[i][j] == 1 and cell_on_border(i, j):
                    ...
                    elif potential_nucleus(i, j) and xDrx != 1:
                        ret = update_recrystallized_cell_state(i, j)
                        if ret != 1:
                            updation_grid.dislocation_densities[i][j] = update_dislocation_density(updation_grid.dislocation_densities[i][j], delta_strain)
                    elif original_grid.dynamic_recrystallization_number[i][j] == 0:
                        updation_grid.dislocation_densities[i][j] = update_dislocation_density(updation_grid.dislocation_densities[i][j], delta_strain)
