...
current_strain = 0   #0.0172 #i_changed
true_strain = []
true_stress = []
global new_grains
new_grains = 0
...
#cell_strain = np.full((n, n), 0.0172)             #i_changed
...
def dislocation_energy(P_prev, orientation_current):
    ...
    #     print("delta strain: ", (strains[k] - strains[k - 1]))
    _, delta_strn = strain(orientation_current)
...
def update_state(P_prev, orientation_current, state_current):
    ...
    if random_number <= nucleation_probability:                                             #i_changed (<=) (previously it was <)
        choice = 1
...
def propagateGrainBoundary(i, j, k):
    ...
    stored_deformation_energy = 0.5 * np.sqrt(original_grid.dislocation_densities[i][j]) * mu(Current_Temp) * (b ** 2)               #i_changed (np.sqrt())
