...
def initialize_dislocation_density():
    ...
    # creates states-2 diff random values only?
...
def mis_orientation(ph_1, th_1, om_1, ph_2, th_2, om_2):
    ...
    R_1 = [[math.cos(om_1) * math.cos(ph_1) - math.sin(om_1) * math.sin(ph_1) * math.cos(th_1),    math.cos(om_1) * math.sin(ph_1) + math.sin(om_1) * math.cos(ph_1) * math.cos(th_1),   math.sin(om_1) * math.sin(th_1)],
           [-math.sin(om_1) * math.cos(ph_1) - math.cos(om_1) * math.sin(ph_1) * math.cos(th_1),  -math.sin(ph_1) * math.sin(om_1) + math.cos(ph_1) * math.cos(om_1) * math.cos(th_1),   math.cos(om_1) * math.sin(th_1)],
           [math.sin(ph_1) * math.sin(th_1),                                                       math.sin(th_1) * math.cos(ph_1),                                                                       math.cos(th_1)]]

    R_2 = [[math.cos(om_2) * math.cos(ph_2) - math.sin(om_2) * math.sin(ph_2) * math.cos(th_2),   math.cos(om_2) * math.sin(ph_2) + math.sin(om_2) * math.cos(ph_2) * math.cos(th_2),   math.sin(om_2) * math.sin(th_2)],
           [-math.sin(om_2) * math.cos(ph_2) - math.cos(om_2) * math.sin(ph_2) * math.cos(th_2),  -math.sin(ph_2) * math.sin(om_2) + math.cos(ph_2) * math.cos(om_2) * math.cos(th_2),   math.cos(om_2) * math.sin(th_2)],
...
def grain_velocity(delta_dislocation_btw_adj_cells, misorientation_btw_adj_cells):                          
    mobility  = Mo * (1 - np.exp( -5 * ((float(misorientation_btw_adj_cells) / critical_misorientation) ** 4)))
...
def update_dislocation_density(P_prev_time_step, delta_strn):                                                                                                                                    
    ...
    # 1st change :- P_prev => P_prev_time_step
    P_new = P_prev_time_step + delta_p_for_same_cell
    return P_new


def update_state(P_prev_same_cell, delta_strn, state_current):
    P_new = update_dislocation_density(P_prev_same_cell, delta_strn)
...
def propagateGrainBoundary(i, j, k):
    ...
    stored_stress = 0.5 * np.sqrt(original_grid.dislocation_densities[i][j]) * mu(Current_Temp) * (b ** 2)
    #do we need this stored_stress line?        
    delta_orientation = mis_orientation(original_grid.orientation[0][0][i][j], original_grid.orientation[0][1][i][j], original_grid.orientation[0][2][i][j], original_grid.orientation[0][0][favoured_indices[0]][favoured_indices[1]], original_grid.orientation[0][1][favoured_indices[0]][favoured_indices[1]], original_grid.orientation[0][2][favoured_indices[0]][favoured_indices[1]])
    #do we need this delta_orientation line?
...
def main():
    ...
    while (k < iterations):
                ...
                if cell_on_border(i, j):
                    for indices in neighbour_indices:
                        ...
                        if original_grid.state[i][j] == original_grid.state[indices[0]][indices[1]]:
                            ...
                            delta_dislocations += [original_grid.dislocation_densities[i][j] - original_grid.dislocation_densities[indices[0]][indices[1]]]
        ...
        N_c += (N_c - N_unchanged_grains)                                                                                    #query
        ...
        true_strain += [net_strain] #[strains[k]]
